...
const orders = {
    orderSummary: { id: 'place-order', enabled: true },
    activeOrders: { id: 'slot-selection', enabled: false }
}
...
export function isEnabled(feature) {
    if (isObject(feature)) {
        ...
        const featureFlag = Object.values(features).find(flag => flag.id === feature);
...
export function flaggedWith(feature, Feature, DisabledFeature = () => null) {
    return props => (isEnabled(feature) ? <Feature {...props} /> : <DisabledFeature {...props} />);
}